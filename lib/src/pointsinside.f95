...
contains
	...
	subroutine pointsinpolygon(coordinates,nc,points,np,triangles,nt,outvalues)
		...
		integer :: i,j, value
		real(kind=8) :: p(3), rmax, rmin, r

		!Regardless of the shape, if the particle is further away from the center than
		!the furthest point in the surface it must be outside
		!similarly if it's closer than the closest point it must be inside

		rmin = norm2(coordinates(1,:))
		rmax = norm2(coordinates(1,:))
		do i=2,nc
			r = norm2(coordinates(i,:))
			if(r<rmin) then
				rmin = r
			end if
		...
		do i=1,nc
			p = coordinates(i,:)
			if(norm2(p)>rmax) then
				value = 0
			else if(norm2(p)<rmin) then
				value = 1
			else
